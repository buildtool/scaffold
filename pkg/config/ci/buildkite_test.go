...
func TestBuildkite_Scaffold_Create_Github(t *testing.T) {
	...
	expected := &buildkite.CreatePipeline{
		...
		ProviderSettings: &buildkite.GitHubSettings{
			TriggerMode:                wrappers.String("code"),
			BuildPullRequests:          wrappers.Bool(true),
			BuildPullRequestForks:      wrappers.Bool(false),
			BuildTags:                  wrappers.Bool(false),
			PublishCommitStatus:        wrappers.Bool(true),
			PublishCommitStatusPerStep: wrappers.Bool(true),
		},
...
func pipeline(hookUrl, badgeUrl, webUrl string) *buildkite.Pipeline {
	return &buildkite.Pipeline{
		BadgeURL: wrappers.String(badgeUrl),
		WebURL:   wrappers.String(webUrl),
		Provider: &buildkite.Provider{WebhookURL: wrappers.String(hookUrl)},
	}
}
